...
std::shared_ptr<Scene> TestScene::LoadScene()
{
    ...
    image.AddComponent<PhysicsComponent>(1.0f, 0.2f, 1.0f, Body::BodyType::Dynamic, glm::vec2(1.0f, 1.0f));

    auto ground = scene->CreateEntity();
    ground.AddComponent<TransformComponent>(glm::vec2(0, -5),
                                           glm::vec2(10, 1), 0.0f);
    ground.AddComponent<SpriteComponent>(&groundSprite);
    ground.AddComponent<PhysicsComponent>(1.0f, 1.0f, 1.0f, Body::BodyType::Static, glm::vec2(1.0f, 1.0f));
